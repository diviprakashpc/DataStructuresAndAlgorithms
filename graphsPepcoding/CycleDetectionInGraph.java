...
public class CycleDetectionInGraph {
	...
	public static boolean detectCycleWithDFS(ArrayList<ArrayList<Integer>> graph, int src , boolean[] visited, int parent) {
	 ...
	 public static boolean detectCycleWithDirectedDFS(ArrayList<ArrayList<Integer>> graph, int src , boolean[] visited, boolean[] dfsvisited) {
	    	
		 visited[src] = true;
		 dfsvisited[src] = true;
		 for(Integer i : graph.get(src)) {
			 if(!visited[i]) {
				 if(detectCycleWithDirectedDFS(graph, i, visited, dfsvisited)) return true;
			 }
...
/* In pepcoding video, sir check if node is already visited after removing it from queue */
/* In striver logic , he checked during adding the neighbours. He checked if any already visited node getting added or not except parent node.*/
